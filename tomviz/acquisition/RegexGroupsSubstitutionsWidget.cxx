  ...
  : QWidget(parent), m_ui(new Ui::RegexGroupsSubstitutionsWidget)
{
  ...
  connect(m_ui->newSubstitutionButton, &QPushButton::clicked, [this]() {
    ...
    for (int i = 0; i < m_substitutions.size(); i++) {
      if (m_substitutions[i].groupName() == newSubstitution.groupName()) {
        m_substitutions[i] = newSubstitution;
        replaced = true;
        break;
      }
    }
  ...
  connect(m_ui->regexGroupsSubstitutionsWidget,
          &QTableWidget::itemDoubleClicked, [this](QTableWidgetItem* item) {
            ...
            this->editRegexGroupSubstitution(this->m_substitutions[row]);
            this->writeSettings();
          });
...
void RegexGroupsSubstitutionsWidget::editRegexGroupSubstitution(
  RegexGroupSubstitution substitution)
{
  ...
  for (int i = 0; i < m_substitutions.size(); i++) {
    if (m_substitutions[i].groupName() == substitution.groupName()) {
      m_substitutions[i] = newSubstitution;
      this->setRegexGroupSubstitution(i, newSubstitution);
    }
  }
}
